...
class MyService(rpyc.Service):
    ...
    def exposed_add_default_ip_route(self):
        ega = rpc(MASTER_SERVER, 'get_fourid_GA', (False,))
        egb = rpc(MASTER_SERVER, 'get_fourid_GB', (False,))
        if my_name != ega and my_name != egb:
            return
            
        hname = ''.join(ch for ch in my_name.split('.')[0] if ch.isalnum())
        cmd="%s --add %s,IP,-,3" % (XROUTE, hname)
        print cmd
