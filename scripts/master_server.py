...
class MasterService(rpyc.Service):
    ...
    def exposed_get_fourid_GA(self, ingress=True):
        if ingress:
            return BACKBONES[FOURID_G_INDEX]
        else:
            return self.orchestrate_fourid_ping()[1]


    def exposed_get_fourid_GB(self, ingress=True):
        if ingress:
            return CLIENTS[FOURID_G_INDEX]
        else:
            return self.orchestrate_fourid_ping()[0]
    ...
    def orchestrate_fourid_ping(self):
        PAIRWISE_PING_LOCK.acquire()
        cur_exp = tuple(CLIENTS)
        if cur_exp in PAIRWISE_PING:
            PAIRWISE_PING_LOCK.release()
        ...
        min_pair = None
        amin = None
        a = BACKBONES[FOURID_G_INDEX]
        while True:
            try:
                out = rpc(a, 'get_ping', (CLIENTS[1:],))[0]
                if float(out[0]) == -1: continue
                if float(out[0]) < min_ping:
                    min_ping = float(out[0])
                    amin = out[1]
        ...
        min_ping = 5000
        bmin = None
        b = CLIENTS[FOURID_G_INDEX]
        while True:
            try:
                out = rpc(b, 'get_ping', (BACKBONES[1:],))[0]
                if float(out[0]) == -1: continue
                if float(out[0]) < min_ping:
                    min_ping = float(out[0])
                    bmin = out[1]
    ...
    def exposed_get_commands(self, host = None):
        ...
        if FOURID_EXPERIMENT:
            ...
            cmd += ["self.exposed_add_default_ip_route()"];
            if CLIENTS:
